...
4. Resolve each skill through the 4-tier resolution chain (see `.factory/skills/resolve-skill/SKILL.md`)
...
Each cell is a comma-separated list of skill directory names from `.factory/skills/`. Use `—` for "not applicable."
...
Verification gates in `workflow-plan-phase` and `workflow-implement-slice` check these conditions. See `.factory/skills/setup-verify/SKILL.md` for the full verification procedure.
