...
4. Matching skills are copied from `.factory/skill-library/` → `.factory/skills/`
...
| `DESIGN_DIRECTION` | `*` (any confirmed value) | Fills `{{PLACEHOLDER}}`s in `.factory/skills/brand-guidelines/SKILL.md` in-place | `brand-guidelines` (in-place fill, not copy) |
| `DESIGN_DIRECTION` | `*cinematic*` OR `*immersive*` | `stack/3d/threejs-pro` | `threejs-pro` |

Note: `DESIGN_DIRECTION` does not copy a skill from the library — it fills placeholders in the pre-placed `.factory/skills/brand-guidelines/SKILL.md` template. Exception: when the direction is `Cinematic/Immersive`, `threejs-pro` is also installed as a standard skill copy, since WebGL and Three.js capabilities will be required.
...
| `OBSERVABILITY` | `*structured-logging*` OR `*pino*` OR `*winston*` | Pre-installed: `.factory/skills/logging-best-practices` | `logging-best-practices` (pre-installed) |
...
| `api` | Pre-installed: `.factory/skills/api-design-principles` | `api-design-principles` (pre-installed) |
...
| Pre-installed: `.factory/skills/brand-guidelines` | Brand color and typography application (pre-installed) |
...
1. Create a directory under the appropriate category: `.factory/skill-library/[category]/[subcategory]/[skill-name]/SKILL.md`
