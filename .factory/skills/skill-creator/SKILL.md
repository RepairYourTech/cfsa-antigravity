...
- Extending the `.factory/skills/` or `.factory/skill-library/` directories
...
Every skill lives in a directory under `.factory/skills/` (installed, always available) or `.factory/skill-library/` (catalog, provisioned per-project):
...
| Is this installed or library? | `.factory/skills/` (always active) vs `.factory/skill-library/` (provisioned) |

**Decision rules:**
- Cross-cutting concerns (planning, debugging, testing) → `.factory/skills/`
- Tech-specific (React, PostgreSQL, Docker) → `.factory/skill-library/stack/`
- Surface-specific (web perf, mobile UX, API design) → `.factory/skill-library/surface/`
- Meta/tooling (git, MCP builders, tmux) → `.factory/skill-library/meta/`
...
If the skill goes in `.factory/skill-library/`, add it to `MANIFEST.md`:
...
If the skill goes in `.factory/skills/`, no registration needed — it's always active.
...
### `.factory/skills/` (Installed Skills)
...
### `.factory/skill-library/` (Library Skills)
