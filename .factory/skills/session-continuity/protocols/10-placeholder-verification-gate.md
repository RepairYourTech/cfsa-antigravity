...
> **Note**: As of v3, workflows no longer use `{{PLACEHOLDER}}` values for skill paths. Instead, they read skill names from the surface stack map in `.factory/instructions/tech-stack.md`. This gate verifies map completeness.
...
Read `.factory/instructions/tech-stack.md` and parse both tables:
...
Additionally verify that `.factory/instructions/commands.md` has non-template values. If the commands section still contains `{{COMMAND_SECTIONS}}`, run `/bootstrap-agents` to fill it.
