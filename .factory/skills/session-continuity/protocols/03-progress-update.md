...
1. **Mark tasks and acceptance criteria `[x]`** in the slice's tracking location (use the exact files in the `.factory/progress/` directory, NEVER guess the path):
   - If slice has its own file: update `.factory/progress/slices/phase-NN-slice-NN.md`
   - If inline: update `.factory/progress/phases/phase-NN.md`
...
5. **Update `.factory/progress/index.md`** — recalculate overall progress:
   ```markdown
