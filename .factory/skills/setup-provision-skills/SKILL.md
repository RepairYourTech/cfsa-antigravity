...
This skill provisions Claude Code skills from `.factory/skill-library/` (shared via symlink to `.factory/skill-library/`) based on `.factory/instructions/tech-stack.md`.
...
1. Read `.factory/instructions/tech-stack.md`
2. Read `.factory/skill-library/MANIFEST.md`
...
1. **Tier 1 (exact):** `.factory/skills/{name}/SKILL.md` exists or exact manifest mapping exists
...
Use `.factory/skills/resolve-skill/SKILL.md` for resolution behavior.

### 3) Provision

For each resolved library path:

1. Copy library directory into `.factory/skills/{installed-name}/`
...
- `.factory/instructions/tech-stack.md` (`{{INSTALLED_SKILLS}}`)
