...
3. **The Tracking**: Progress markdown files in `.factory/progress/` are updated.
...
5. **The Memory**: All three memory files in `.factory/memory/` are checked:
...
6. **The Session Log**: A session close log exists in `.factory/progress/sessions/`.
   - Follow `.factory/skills/session-continuity/protocols/05-session-close.md` — write what was accomplished, deferred, and where the next session should start
7. **The Next Step**: The user is told what comes next in the pipeline.
   - Read the pipeline progress file (`.factory/progress/spec-pipeline.md` or equivalent)
