...
pipeline:
  ...
  stage: maintenance
  predecessors: [] # on-demand
  successors: [bootstrap-agents] # re-run bootstrap after sync
  skills: [git-workflow]
  calls-bootstrap: true # re-bootstrap after sync to fill any new placeholders
---
...
**Input:** Path to the upstream kit (default: `~/spec-pipeline-starter`)
