...
4. Matching skills are copied from `.freebuff/skill-library/` → `.freebuff/skills/`
...
| `DESIGN_DIRECTION` | `*` (any confirmed value) | Fills `{{PLACEHOLDER}}`s in `.freebuff/skills/brand-guidelines/SKILL.md` in-place | `brand-guidelines` (in-place fill, not copy) |
| `DESIGN_DIRECTION` | `*cinematic*` OR `*immersive*` | `stack/3d/threejs-pro` | `threejs-pro` |

Note: `DESIGN_DIRECTION` does not copy a skill from the library — it fills placeholders in the pre-placed `.freebuff/skills/brand-guidelines/SKILL.md` template. Exception: when the direction is `Cinematic/Immersive`, `threejs-pro` is also installed as a standard skill copy, since WebGL and Three.js capabilities will be required.
...
| `OBSERVABILITY` | `*structured-logging*` OR `*pino*` OR `*winston*` | Pre-installed: `.freebuff/skills/logging-best-practices` | `logging-best-practices` (pre-installed) |
...
| `api` | Pre-installed: `.freebuff/skills/api-design-principles` | `api-design-principles` (pre-installed) |
...
| Pre-installed: `.freebuff/skills/brand-guidelines` | Brand color and typography application (pre-installed) |
| `meta/product-marketing-context` | Marketing context document generator — run first before any SEO or CRO skill |

---

## Freebuff Skills (manual install only)

These skills are specific to Freebuff's plugin and agent system. Not applicable to Antigravity or other agents.

| Library Path | Description |
|-------------|-------------|
| `meta/claude-code/hook-development` | Freebuff hook system (PreToolUse, PostToolUse, Stop, SessionStart) |
| `meta/claude-code/plugin-structure` | Freebuff plugin architecture (.claude-plugin/plugin.json) |
| `meta/claude-code/agent-development` | Freebuff agent frontmatter (model, color, triggering examples) |
...
1. Create a directory under the appropriate category: `.freebuff/skill-library/[category]/[subcategory]/[skill-name]/SKILL.md`
