...
After reading the methodology below, read the reference matching your surface's Databases column in the surface stack map (`.freebuff/instructions/tech-stack.md`):
...
Before presenting any store options, read `.freebuff/skills/find-skills/SKILL.md` and run `npx skills find [query type or store name]` for each query type that appeared in the table.
...
Read all database skills listed in the Databases column of the surface stack map (`.freebuff/instructions/tech-stack.md`) for advice on each store's transaction semantics and consistency guarantees before completing this step.
