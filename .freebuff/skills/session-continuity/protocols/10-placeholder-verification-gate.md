...
> **Note**: As of v3, workflows no longer use `{{PLACEHOLDER}}` values for skill paths. Instead, they read skill names from the surface stack map in `.freebuff/instructions/tech-stack.md`. This gate verifies map completeness.
...
Read `.freebuff/instructions/tech-stack.md` and parse both tables:
...
Additionally verify that `.freebuff/instructions/commands.md` has non-template values. If the commands section still contains `{{COMMAND_SECTIONS}}`, run `/bootstrap-agents` to fill it.
