...
This skill provisions Freebuff skills from `.freebuff/skill-library/` (shared via symlink to `.freebuff/skill-library/`) based on `.freebuff/instructions/tech-stack.md`.
...
1. Read `.freebuff/instructions/tech-stack.md`
2. Read `.freebuff/skill-library/MANIFEST.md`
...
1. **Tier 1 (exact):** `.freebuff/skills/{name}/SKILL.md` exists or exact manifest mapping exists
...
Use `.freebuff/skills/utilities/resolve-skill.md` for resolution behavior.

### 3) Provision

For each resolved library path:

1. Copy library directory into `.freebuff/skills/{installed-name}/`
...
- `.freebuff/instructions/tech-stack.md` (`{{INSTALLED_SKILLS}}`)
