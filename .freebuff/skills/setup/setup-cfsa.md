---
name: setup-cfsa
description: Main setup workflow for Freebuff CFSA pipeline — gather project info, fill placeholders, provision skills, verify installation
parameters:
  - name: project_name
    type: string
    required: true
...
The setup-cfsa skill is the main entry point for configuring the CFSA pipeline in Freebuff. It gathers project information, fills global placeholders, provisions skills from the skill library, and verifies the installation is ready for pipeline execution.

This is the Freebuff equivalent of Antigravity's `/bootstrap-agents` workflow.

## Prerequisites

1. Freebuff is installed and project is open
2. `.freebuff/` directory structure exists (created by `cfsa-antigravity init --agent claude`)
...
**What gets filled:**
- Per-Surface Skills table in `.freebuff/instructions/tech-stack.md`
...
2. For each skill name in every cell:
   ...
   - Install matching skills from `.freebuff/skill-library/`
...
```
✅ CFSA Pipeline is ready for Freebuff!
...
Documentation:
- .freebuff/README.md — Freebuff version guide
...
- `.freebuff/skills/setup/setup-fill-placeholders.md` — Surface Stack Map filling
- `.freebuff/skills/setup/setup-provision-skills.md` — Skill provisioning
- `.freebuff/skills/setup/setup-verify.md` — Installation verification
- `.freebuff/skills/utilities/resolve-skill.md` — 4-tier resolution chain
