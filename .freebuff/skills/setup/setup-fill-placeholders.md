...
This skill surgically fills `{{PLACEHOLDER}}` values across the Freebuff kit. It is idempotent — already-filled placeholders are not overwritten, and new values are only applied where explicitly provided.
...
| `CONTRACT_LIBRARY` (also fills `.freebuff/rules/tdd-contract-first.md`) | Rule templates |
| `VALIDATION_COMMAND` | `.freebuff/instructions/commands.md`, root configs |
| `FRAMEWORK_PATTERNS` | `.freebuff/instructions/patterns.md` |
| `PROJECT_STRUCTURE`, `ARCHITECTURE_TABLE` | `.freebuff/instructions/structure.md` |
...
**Target**: `.freebuff/instructions/tech-stack.md`
...
**Target**: `.freebuff/instructions/tech-stack.md` → Cross-Cutting Skills table
...
**Target**: `.freebuff/instructions/tech-stack.md` → Global Settings table
...
Scan `.freebuff/rules/*.md` for `{{PLACEHOLDER}}` values:
...
Scan all `.freebuff/skills/*/SKILL.md` for `{{PLACEHOLDER}}` values and fill matches:
