...
```bash
node .freebuff/skills/impeccable/scripts/load-context.mjs
```
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node .freebuff/skills/impeccable/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.
...
Optionally STOP and call the AskUserQuestion tool to clarify. Ask whether they'd like a brief summary of PRODUCT.md appended to FREEBUFF.md for easier agent reference. If yes, append a short **Design Context** pointer section there.
