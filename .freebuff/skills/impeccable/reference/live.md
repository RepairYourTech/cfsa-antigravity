...
Harness policy:
- **Freebuff**: run the poll as a **background task** (no short timeout). The harness notifies you when it completes, so the main conversation stays free. Do not block the shell.
...
```bash
node .freebuff/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node .freebuff/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node .freebuff/skills/impeccable/scripts/live-status.mjs
node .freebuff/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .freebuff/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .freebuff/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .freebuff/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .freebuff/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .freebuff/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node .freebuff/skills/impeccable/scripts/detect-csp.mjs
```
