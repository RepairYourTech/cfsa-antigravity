...
- Extending the `.freebuff/skills/` or `.freebuff/skill-library/` directories
...
Every skill lives in a directory under `.freebuff/skills/` (installed, always available) or `.freebuff/skill-library/` (catalog, provisioned per-project):
...
| Is this installed or library? | `.freebuff/skills/` (always active) vs `.freebuff/skill-library/` (provisioned) |

**Decision rules:**
- Cross-cutting concerns (planning, debugging, testing) → `.freebuff/skills/`
- Tech-specific (React, PostgreSQL, Docker) → `.freebuff/skill-library/stack/`
- Surface-specific (web perf, mobile UX, API design) → `.freebuff/skill-library/surface/`
- Meta/tooling (git, MCP builders, tmux) → `.freebuff/skill-library/meta/`
...
If the skill goes in `.freebuff/skill-library/`, add it to `MANIFEST.md`:
...
If the skill goes in `.freebuff/skills/`, no registration needed — it's always active.
...
### `.freebuff/skills/` (Installed Skills)
...
### `.freebuff/skill-library/` (Library Skills)
