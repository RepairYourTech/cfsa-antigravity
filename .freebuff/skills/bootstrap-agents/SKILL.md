...
1. **Fill the surface stack map** in `.freebuff/instructions/tech-stack.md` with stack decisions
...
**Output**: Filled map cells + newly installed skills in `.freebuff/skills/`
...
Open `.freebuff/instructions/tech-stack.md` and update:
...
### `.freebuff/instructions/commands.md`
...
Scan `.freebuff/skills/*/SKILL.md` and `.freebuff/rules/*.md` for `{{PLACEHOLDER}}` values and fill any that match the provided values. Currently applicable:
...
### Tier 1: Exact Match
Check `.freebuff/skill-library/{name}/` (or `.freebuff/skills/{name}/` if already installed).
- **Found in library** AND not yet installed → copy to `.freebuff/skills/{name}/`, fill any `{{PLACEHOLDER}}`s in the copied SKILL.md
- **Already installed** → skip (idempotent)
- **Not found** → proceed to Tier 2

### Tier 2: Partial Match + Adequacy Check
Search `.freebuff/skill-library/` and `.freebuff/skills/` for skills whose name contains the base term. E.g., for `surrealdb-embedded`, check if `surrealdb` exists.
...
Read `.freebuff/skills/find-skills/SKILL.md` and invoke its discovery methodology to search for the skill externally.
...
- **Already-installed skills**: Skills that already exist in `.freebuff/skills/` are not re-copied from the library
