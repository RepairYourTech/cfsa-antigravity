...
- e94c88a: Remove the legacy memory migrator entirely.
...
- 362e682: Remove the `mirror-plans.mjs` spec-mirroring system entirely.
...
- 9087b00: Make the Claude runtime standalone instead of a thin wrapper over the Antigravity runtime.
...
- f0b6c17: Add mandatory ideation context reload to all 5 create-prd shards

  - **Critical**: `create-prd-stack` — add domain deep dive loading before tech decisions; MoSCoW bullets are insufficient context for framework choices
  - **Critical**: `create-prd-architecture` — add full ideation reload (index, constraints, CX files, domain indexes) before component/data design
  - **Critical**: `create-prd-security` — add role matrix and CX trust boundary loading before access control design
  - **High**: `create-prd-design-system` — add heavy domain workflow loading for navigation and archetype decisions
  - **High**: `create-prd-compile` — add domain depth context for phase complexity sizing

## 2.14.0

### Minor Changes

- 0523554: Add comprehensive enforcement gates across 18 pipeline workflows

  - **Critical**: Add Completion Gate (memory capture) to 12 workflows that previously lacked mandatory progress tracking
  - **High**: Add event consumer cross-reference verification, deep dive completeness checks, feature ledger integrity at scaffold/verification/validation stages, domain-to-shard coverage verification, and BOUNDARY stub auditing at phase validation
  - **Medium**: Add post-write verification, shard failure recovery protocol, session continuity in setup-workspace, next-step restrictions, spec coverage pre-append verification, and post-ideation file count verification
  - Add deep ideation loading protocol and feature ledger protocol as new skill references
  - Add spec complexity gates and shard count guidance to decomposition workflows
  - Add split spec discovery and companion file awareness to implement-slice-setup

## 2.13.4

### Patch Changes

- 46f219f: fix: add Completion Gate enforcement to 4 create-prd shard workflows
